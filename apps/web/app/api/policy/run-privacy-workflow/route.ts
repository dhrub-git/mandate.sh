...
import { privacySecurityGraph } from "@repo/agents";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log("=== INCOMING REQUEST DATA (Privacy & Security) ===");
    ...
    const events = await privacySecurityGraph.streamEvents(
      { pdf_path: filePath },
      { version: "v2" },
    );
    ...
    const stream = createUIMessageStream({
      execute: async ({ writer }) => {
        ...
        try {
          ...
          console.error("Privacy & Security workflow error:", err);

          writer.write({
            type: "error",
            errorText: `Privacy & Security workflow failed: ${err.message}`,
          });
        } finally {
          // Cleanup temp file
          if (filePath) {
