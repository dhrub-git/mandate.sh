import { communityBenefitGraph } from "@repo/agents";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.log("=== INCOMING REQUEST DATA (Community Benefit) ===");
    ...
    const events = await communityBenefitGraph.streamEvents(
      { pdf_path: filePath },
      { version: "v2" },
    );
    ...
    const stream = createUIMessageStream({
      execute: async ({ writer }) => {
        ...
        try {
          ...
          console.error("Community benefit workflow error:", err);

          writer.write({
            type: "error",
            errorText: `Community benefit workflow failed: ${err.message}`,
          });
        } finally {
          // Cleanup file
          if (filePath) {
